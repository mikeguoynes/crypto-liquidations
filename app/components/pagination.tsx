import { PaginationFilters } from "~/routes/liquidations"

/* eslint-disable jsx-a11y/anchor-is-valid */
type PageChangeProps = {
  onPageChange: (pageChangeDirection: string) => void,
  filters: PaginationFilters,
  totalCount: number,
}

export function Pagination(props: PageChangeProps) {
    return (
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Showing <span className="font-medium">{props.filters?.offset + 1}</span> to <span className="font-medium">{props.filters?.offset + props.filters.limit }</span> of <span className="font-medium">{ props?.totalCount } </span>results
              </p>
            </div>
            <div>
              ...
              <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a onClick={() => props.onPageChange('previous')} href="#" className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                ...
                <a onClick={() => props.onPageChange('next')} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
